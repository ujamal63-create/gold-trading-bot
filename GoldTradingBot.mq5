...
void CleanupTradeTimes(){datetime now=TimeCurrent(); datetime f[]; ArrayResize(f,0); for(int i=0;i<ArraySize(tradeTimes);i++){if((now-tradeTimes[i])<=86400){int n=ArraySize(f); ArrayResize(f,n+1); f[n]=tradeTimes[i];}} tradeTimes=f;}
