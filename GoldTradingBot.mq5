...
input int              InpSRLookbackBars             = 120;
...
bool GetSupportResistance(double &support, double &resistance){
   int bars=InpSRLookbackBars;
   int w=InpSRWindow;
   if(bars<25) bars=25;
   if(w<1) w=1;
   MqlRates rates[];
   if(CopyRates(InpSymbol,InpTimeframe,0,bars,rates)<(w*2+5)) return false;
   ArraySetAsSeries(rates,true);
   double highs[], lows[];
   ArrayResize(highs,ArraySize(rates)); ArrayResize(lows,ArraySize(rates));
   for(int i=0;i<ArraySize(rates);i++){ highs[i]=rates[i].high; lows[i]=rates[i].low; }

   double price=(SymbolInfoDouble(InpSymbol,SYMBOL_BID)+SymbolInfoDouble(InpSymbol,SYMBOL_ASK))*0.5;
   support=0.0; resistance=0.0;
   double bestBelowDist=DBL_MAX, bestAboveDist=DBL_MAX;

   for(int i=w+2;i<ArraySize(rates)-w-1;i++){
      if(IsSwingLow(lows,i,w)){
         double lv=lows[i];
         if(lv<price){ double d=price-lv; if(d<bestBelowDist){ bestBelowDist=d; support=lv; }}
      }
      if(IsSwingHigh(highs,i,w)){
         double lv=highs[i];
         if(lv>price){ double d=lv-price; if(d<bestAboveDist){ bestAboveDist=d; resistance=lv; }}
      }
   }
...
TrendDirection GetSignal(){
   ...
   if(!GetSupportResistance(support,resistance)) return TREND_NONE;

   double atr=GetATRValue();
   if(atr<=0.0) return TREND_NONE;
   ...
   bool nearSupport=(support>0.0 && MathAbs(mid-support)<=srBuffer);
   bool nearResistance=(resistance>0.0 && MathAbs(mid-resistance)<=srBuffer);
   bool roomToResistance=(resistance>0.0 && (resistance-mid)>atr*0.8);
   bool roomToSupport=(support>0.0 && (mid-support)>atr*0.8);

   if(adx[0]>=InpADXTrendThreshold){
      if(f[0]>s[0]&&pdi[0]>mdi[0]&&r[0]>=InpRSIBuyThreshold&&(nearSupport||roomToResistance)) return TREND_BUY;
      if(f[0]<s[0]&&mdi[0]>pdi[0]&&r[0]<=InpRSISellThreshold&&(nearResistance||roomToSupport)) return TREND_SELL;
   }
   return TREND_NONE;
}
